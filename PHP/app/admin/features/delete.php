...
require_once "/app/requests/features.php"; // Contient des fonctions pour effectuer des opérations sur les articles.
...
$feature = findOneFeatureById(isset($_POST['id']) ? $_POST['id'] : 0);
...
if (hash_equals($_SESSION['token'], isset($_POST['token']) ? $_POST['token'] : '')) {
    ...
    if (deleteFeature($feature['id'])) {

        if ($feature['imageName']) {
            deleteImage($feature['imageName'], 'feature');
        }

        // Si la suppression est réussie, stocke un message de succès dans la session.
        $_SESSION['messages']['success'] = "feature supprimé avec succès";
    } else {
